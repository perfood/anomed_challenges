...
# train model
df_features_train = np.asarray(df_features_train).astype(np.float32)
df_glucose_train_in = np.asarray(df_glucose_train_in).astype(np.float32)
df_glucose_train_out = np.asarray(df_glucose_train_out).astype(np.float32)
df_features_test = np.asarray(df_features_test).astype(np.float32)
df_glucose_test_in = np.asarray(df_glucose_test_in).astype(np.float32)
df_glucose_test_out = np.asarray(df_glucose_test_out).astype(np.float32)

X_train, X_val, X_train_1d, X_val_1d, Y_train, Y_val = train_test_split(df_features_train, df_glucose_train_in,
                                                                        df_glucose_train_out, test_size=0.2,
                                                                        random_state=0)
...
test_loss, test_mae = model.evaluate([df_features_test, df_glucose_test_in], df_glucose_test_out, verbose=0)
